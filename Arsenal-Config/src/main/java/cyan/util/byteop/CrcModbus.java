...
public class CrcModbus {
	...
	for (int i = 0; i < usDataLen; i++) {    
	    ...
	    uchCRCHi = (byte) (uchCRCLo ^ CrcModbus.auchCRCHi[uIndex]);    
	    uchCRCLo = CrcModbus.auchCRCLo[uIndex];   
	}     
    ...
    /*==========  ==========*/
    static public int calculate(byte[] buff, int start, int end ){
	// Set Default Value
	byte crcH = (byte) 0xFF;   
	byte crcL = (byte) 0xFF;  
	//    
	for (int i = start; i < end; i++) {    
	    int uIndex = (crcH ^ buff[i]) & 0xff;      
	    crcH = (byte) (crcL ^ CrcModbus.auchCRCHi[uIndex]);    
	    crcL = CrcModbus.auchCRCLo[uIndex];   
	}  
	return ((((int) crcH) << 8 | (((int) crcL) & 0xFF))) & 0xFFFF;
    }
